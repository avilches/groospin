 ...
 */
class ArcadeSetAT extends ArcadeSet {
    J2K.Preset preset
    int player1 = 1
    int player2 = 2
    ...
    void exit(key) { preset.buttonToKey(player1, 8, key) }

    void pinballLeft(key) { preset.buttonToKey(player1, 9, key) }

    void pinballRight(key) { preset.buttonToKey(player1, 10, key) }
    ...
    void p2Action5(key) { preset.buttonToKey(player2, 9, key) }

    void p2Action6(key) { preset.buttonToKey(player2, 10, key) }

    void p2Start(key) { preset.buttonToKey(player2, 11, key) }

    void coin(key) { preset.buttonToKey(player2, 12, key) }
