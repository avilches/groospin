...
"""
        ...
        websiteSystems << "<table id='systems'><thead>\n<tr>\n    <td>#</td><td>Tipo</td><td>Sistema</td><td>Roms</td><td>Tamaño</td><td>Wheels</td><td>Video</td><td>Temas</td><td>Tamaño medias</td>\n</tr>\n</thead>\n<tbody>"
    }
    ...
    void endSystem(CheckTotalResult checkResult) {
        ...
        websiteSystems << "<tr>\n    <td class='n'>${++n}</td><td class='group'>${checkResult.group}</td><td class='system'><a href='/sistema/${sanitize(checkResult.system.name)}/'>${checkResult.system.name}</a><div class='emu'>${checkResult.system.defaultEmulator.name?:""}</div></td><td class='roms'>${checkResult.totalRoms}</td><td class='romSize'>${humanReadableByteSize(checkResult.totalRomSize)}</td>" +
